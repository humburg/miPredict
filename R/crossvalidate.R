...
crossvalidate <- function(imputed, outcome, k=10, force=FALSE, ...){
  ...
  imp_test <- complete(imputed, action="long") %>% mutate(fold=rep(data_orig$fold, imputed$m), prediction=NA, se=NA) %>% 
    mutate(!!outcome := forcats::fct_recode(.data[[outcome]], NULL='(Missing)'))
  for(i in 1:k){
    ...
    imp_pred <- predict(fit$pooled_model, newdata=filter(imp_test, .data$fold==i), type="response", se.fit=TRUE)
